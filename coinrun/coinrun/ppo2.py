...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        clipfrac_train = tf.reduce_mean(tf.cast(tf.greater(tf.abs(ratio_train - 1.0), CLIPRANGE), dtype=tf.float32))
        ...
        if Config.SNI or Config.SNI2:
            ...
            clipfrac_run = tf.reduce_mean(tf.cast(tf.greater(tf.abs(ratio_run - 1.0), CLIPRANGE), dtype=tf.float32))
        else:
